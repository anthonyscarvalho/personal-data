...
import { UserLoginRequest, UserRegisterRequest, GetUserRequest } from './schema/user.api.request';
import {
  hashPassword,
  isEmailAddress,
  isPhoneNumber,
...
import { DefaultUserProjection, User } from './schema/user.dto';
...
export class UserService extends BaseService {
  ...
  async login(request: UserLoginRequest) {
    ...
    if (request.email && isEmailAddress(request.email)) {
      ...
      throw new BadRequestException(
        this.getLocalization('login', 'no_login_identifier'),
      );
    }
    ...
    if (!user) {
      throw new UnauthorizedException(
        this.getLocalization('login', 'invalid_credentials'),
      );
    }
    ...
    if (user.isBlocked) {
      throw new UnauthorizedException(
        this.getLocalization('login', 'user_blocked'),
      );
    }
  ...
  async register(request: UserRegisterRequest) {
    ...
    if (user) {
      throw new BadRequestException(
        this.getLocalization('register', 'user_email_already_exists'),
      );
    }
  ...
  async updatePassword(userId: string, newPassword: string): Promise<boolean> {
    try {
      ...
      throw new BadRequestException(
        this.getLocalization('password-updated-failure', 'password_update_failure'),
      );
    }
  }
  ...
  async updateProfileStatusByEmail(email: string, newStatus: number): Promise<boolean> {
    try {
      ...
      if (!userToUpdate) {
        throw new BadRequestException(
          this.getLocalization('profile-status', 'no_login_identifier'),
        );
      }
      ...
      throw new BadRequestException(
        this.getLocalization('profile-status', 'profile_status_update_failure'),
      );
    }
  }
